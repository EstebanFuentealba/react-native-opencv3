...
public class CvCameraView extends JavaCameraViewFixed implements CvCameraViewListener2Fixed {
    ...
    private boolean                mUseMRZDetection   = false;
    ...
    public void setUseMRZDetection(boolean value) {
        this.mUseMRZDetection = value;
    }
    ...
    public Mat onCameraFrame(CameraBridgeViewBaseFixed.CvCameraViewFrame inputFrame) {
        ...
        if(mUseMRZDetection) {
            ...
            Imgproc.Sobel(dst, dst, CvType.CV_8U, 1, 0, 1);
            ...
            for (int i = 0; i < contours.size(); ++i) {
                ...
                if (ar > 5.0) {
                    ...
                    if(w > 340 && h > 60){
                        response.putInt("ar", ar);
                        response.putInt("x", x);
                        response.putInt("y", y);
                        response.putInt("width", w);
                        response.putInt("height", h);
                        ...
                        try {
                            ...
                            response.putString("cropped", toBase64(bitmap, 100));
            ...
            if(mrzAreaFound) {
                ...
                Log.d(TAG, "payload is: " + response.toString());
                mContext.getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class)
                        .emit("onMRZDetected", response);
            }
